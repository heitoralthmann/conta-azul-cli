...
final class AccessTokenServiceTest extends TestCase
{
    ...
    private function config(?string $bootstrap=NULL): Configuration {
    ...
    public function testRefreshesAndPersistsAnExpiringToken(): void {
        $stored = new TokenData('old-access', new \DateTimeImmutable('-1 minute'), 'old-refresh', new \DateTimeImmutable('-1 hour'));
        $fresh  = new TokenData('new-access', new \DateTimeImmutable('+1 hour'), 'new-refresh', new \DateTimeImmutable('now'));
        $repo   = new class($stored) implements TokenRepositoryInterface {
            public ?TokenData $saved = NULL;


            public function __construct(private ?TokenData $token) {
            }


            public function save(TokenData $token): void {
 $this->saved = $token; $this->token = $token; }
